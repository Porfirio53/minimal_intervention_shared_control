...
| `tau_H` 与逐障碍 `tau^S_{j,k}` | `network/timestamp_buffer.py`; `safety/robust_cbf_qp.py::RelativeStateObservation`; `sim2d/simulation.py` | 两种年龄由不同对象持有，安全接口逐障碍接收 `age` | Conflict：安全层使用单一标量 `tau_s` | 是 | 仿真场景目前给各障碍相同的传感器年龄，但接口和测试允许彼此不同 |
| 四类轨迹：人工意图、自主、共享执行、障碍 | `risk/prediction_tube.py::PredictionRiskTube`; `sim2d/simulation.py::_build_prediction_problem` | 四种对象使用独立字段和类型；共享执行轨迹由混合控制经动力学/线性化传播 | Missing/Conflict：没有结构化风险管，未来人工路径被当作 oracle | 是 | 无训练模型时人工候选采用因果常值保持，`Sigma_H=0`；这是明确的简化，不代表概率意图预测已校准 |
...
| 式(4)：控制层混合后传播 | `risk/prediction_tube.py::blend_controls, trajectory_positions`; `sim2d/simulation.py` | `u=u_H+alpha(u_A-u_H)` 后通过车辆模型传播 | Exact | 否 | 未进行位置轨迹逐点插值 |
...
| 式(6)：预测风险管 `T_k` | `risk/prediction_tube.py::PredictionRiskTube`; `_build_prediction_problem` | 保存 `E_H,p_A,E_E,E_O,C,U` 及三类安全裕量 | Missing | 是 | 当前对象是每次固定线性化参考处的风险管快照，不表示整段闭环联合置信区域 |
...
| 式(10)：半空间机会裕量 | `risk/chance_constraint.py::chance_margin`; `_build_prediction_problem` | 固定单位法向，距离中加入 `kappa*sqrt(n^T Sigma n)` | Exact/Equivalent | 是，补显式法向 | 标准差而非方差进入裕量；固定法向是论文指定的局部凸化条件 |
| 式(13)：共享输入关于 `alpha` 仿射 | `risk/prediction_tube.py::blend_controls` | 向量化实现同一公式 | Equivalent | 否 | 无额外残差控制 `r_i` |
| 式(14)–(17)：局部线性化、`S` 与 `g` | `authority/linearization.py::affine_state_prediction`; `authority/sensitivity.py::project_position_rows`; `_build_prediction_problem` | 使用论文 Jacobian 凝聚为 `x=offset+S alpha`，再投影 `g=n^T S` | Conflict：使用非线性有限差分，且每次固定全自主参考 | 是 | 成功求解后移位上一可行 `alpha` 作为下一参考；当前只做一次顺序凸化 |
| 线性化启动参考 | `sim2d/simulation.py::run_simulation` | 首周期使用全自主序列，之后移位上一可行解 | Unsupported assumption | 部分修改 | 论文没有规定“尚无上一可行轨迹”时的初始化；全自主启动需作者确认 |
...
| 式(19)：次级目标 | `authority/lexicographic_qp.py::secondary_objective_matrices` | 实现控制权平滑、相对人工输入修改及极小唯一性正则项 | Conflict/Approximation | 是 | 删除无论文依据的线速度差目标；意图轨迹保持和任务性能项尚未启用，因为权重、参考及校准 `Sigma_H` 未给出 |
...
| 上层不可行处理 | `LexicographicAuthority.solve`; `sim2d/simulation.py` | 显式保存 stage 状态和不可行率；不增加松弛；把标记为不可行的名义命令交给安全层 | Conflict：曾静默返回 `alpha=0` 作为普通解 | 是 | 当前平台后备仍以 `alpha=0` 生成名义命令；论文没有唯一指定上层不可行后的名义命令，需作者确认 |
...
1. 人工意图在未加载训练模型时采用收到命令的因果常值保持，位置协方差为零；`HumanAR` 和行为克隆模块存在，但尚未用 SCAND 数据完成训练、校准和主仿真接线。
2. 障碍预测采用短时单峰常速度高斯模型，协方差按配置增长；只适用于当前恒速动态障碍仿真。
...
- 全量测试：`44 passed`，语句覆盖率 `85%`。
- 静态与格式：Ruff check 通过，60 个 Python 文件格式检查通过，`git diff --check` 通过。
- 构建与导入：`compileall`、核心包导入及 `pip check` 通过。
- 配对 smoke：四个场景、五种方法、相同 seed 17、每次 3 s，共 20 次运行；均未碰撞，不能据此推断完整任务成功率或方法优劣。
- 上层状态：对需要优化的 720 次更新，421 次 `solved`、293 次 `primal infeasible`、6 次 `maximum iterations reached`。后两类均被当作显式不可行/后备状态，没有伪装成可行解。
- 末级状态：3000 个执行步中，2182 次 `solved`、650 次 `primal infeasible`、168 次 `maximum iterations reached`。无解时执行停止后备，但停止不被记录为安全证书。
- 安全隔离：相同 `u_N` 的 3 s `network_anomaly` 重放中，无过滤发生碰撞；standard 和 `tau^S=0/0.05/0.1/0.2 s` robust 运行未碰撞。对应硬过滤器不可行率约为 43.3%、52.0%、52.0%、52.7%、54.7%。
- 数值/物理扫描：全部核心状态、候选控制、名义/过滤控制、`alpha`、年龄、距离、`H-`、预测裕量和求解时间无 NaN/Inf；`alpha` 及最终控制满足配置边界。
